...
globalDef, 
// rules:
rule, keyframes, } from '@cssfn/cssfn'; // cssfn core
// others libs:
 ...
 */
const createCssConfig = (initialProps, options) => {
    // settings:
    const settings = {
        ...options,
    ...
    const getProps = () => {
        if (!_propsCache) {
            _propsCache = ((typeof (initialProps) === 'function') ? initialProps() : initialProps);
        } // if
        return _propsCache;
    };
    // data generates:
     ...
     */
    const keyframesRef = (keyframesName) => {
     ...
     */
    const transformDuplicates = (srcProps, refProps, propRename) => {
        /**
         ...
         */
        const isTransformableProp = (srcPropValue) => {
            if ((srcPropValue === undefined) || (srcPropValue === null))
                return false; // skip empty prop
            if ((typeof (srcPropValue) === 'string') && (/^(none|unset|inherit|initial|revert)$/).test(srcPropValue))
                return false; // ignore global keywords
         ...
         */
        const isSelfProp = (srcPropName, refPropName) => {
            if (!Object.is(srcProps, refProps))
                return false; // if `srcProps` & `refProps` are not the same object in memory => always return `false`
         ...
         */
        const isKeyframesName = (srcPropName) => srcPropName.match(/(?<=@keyframes\s+)[\w-]+/)?.[0] ?? null;
        const isExistingKeyframes = (refPropValue) => {
            if (typeof (refPropValue) !== 'object')
                return false; // should be an object
            if (Array.isArray(refPropValue))
                return false; //   but not an array
         ...
         */
        const deepEqual = (srcPropValue, refPropValue) => {
            if (Object.is(srcPropValue, refPropValue))
                return true; // shallow equal
            if (isExistingKeyframes(refPropValue))
                return false; // `@keyframes` must be compared by reference, no deep equal
            // both must be an object:
            if (typeof (srcPropValue) !== 'object')
                return false;
            if (typeof (refPropValue) !== 'object')
                return false;
            // both must be an array -or- both must not be an array:
            if (Array.isArray(srcPropValue) !== Array.isArray(refPropValue))
                return false;
            // both props count must be the same:
            if (Object.keys(srcPropValue).length !== Object.keys(refPropValue).length)
                return false;
            for (const [deepPropName, deepSrcPropValue] of Object.entries(srcPropValue)) {
                if (!deepEqual(deepSrcPropValue, refPropValue[deepPropName]))
                    return false; // the same prop name with different prop value => false
            } // for
         ...
         */
        const findEqualProp = (srcPropName, srcPropValue) => {
            for (const [refPropName, refPropValue] of Object.entries(refProps)) { // search for duplicates
                if ((refPropValue === undefined) || (refPropValue === null))
                    continue; // skip empty ref
                if (isSelfProp(srcPropName, refPropName))
                    break; // stop search if reaches current entry (search for prev entries only)
                if (!isTransformableProp(srcPropValue))
                    continue; // skip non transformable prop
                // comparing the `srcPropValue` & `refPropValue` deeply:
                if (deepEqual(srcPropValue, refPropValue))
                    return ref(refPropName); // return the link to the ref
         ...
         * -or- `null` if no equivalent found.
         */
        const findEqualKeyframes = (srcKeyframesValue) => {
            for (const refKeyframesValue of Object.values(genKeyframes)) { // search for duplicates
                ...
                if (deepEqual(srcKeyframesValue, refKeyframesValue))
                    return refKeyframesValue; // return the object reference to the ref
            } // for // search for duplicates
            return null; // not found
        };
        /**
         ...
         */
        const modifSrcProps = {}; // initially empty (no modification)
        for (const [srcPropName, srcPropValue] of Object.entries(srcProps)) { // walk each entry in `srcProps`
            if ((srcPropValue === undefined) || (srcPropValue === null))
                continue; // skip empty src
            //#region handle `@keyframes foo`
            {
                const keyframesName = isKeyframesName(srcPropName);
                if (keyframesName) {
                    let srcKeyframesValue = srcPropValue; // assumes the current `srcPropValue` is a valid `@keyframes`' value.
            ...
            if (Array.isArray(srcPropValue)) {
                ...
                const srcLiteralProps = srcPropValue;
    ...
    const rebuild = () => {
        ...
        */
        for (const keyframesValue of Object.values(genKeyframes)) {
            if ((keyframesValue === undefined) || (keyframesValue === null))
                continue; // skip empty keyframes
            /*
                PropEx.Keyframes
                Dictionary<Style>
                -------:---------------
            ...
            */
            for (const [key, frame] of Object.entries(keyframesValue)) {
                ...
                keyframesValue[key] = transformDuplicates(/*srcProps: */ frame, /*refProps: */ props) ?? frame;
            } // for
        } // for
    ...
    const refresh = (immediately = false) => {
        if (immediately) {
            ...
            Promise.resolve().then(() => {
                if (_valid)
                    return; // has been previously generated => abort
                rebuild();
                _valid = true; // mark the `genProps` & `genKeyframes` as valid
            });
        } // if
     ...
     */
    const setDirect = (propName, newValue) => {
        const props = getProps();
    ...
    const getPropDescDecl = (propName) => {
        const propDecl = getDecl(propName);
        if (!propDecl)
            return undefined; // not found
        return {
...
export const usesGeneralProps = (cssProps) => {
    ...
    for (const [propName, propValue] of Object.entries(cssProps)) {
         ...
         * fontSizeXl
         */
        if ((/[a-z](Xs|Sm|Nm|Md|Lg|Xl|Xxl|Xxxl|[0-9]+em)$/).test(propName))
            continue; // exclude
        // suffixes:
        /**
         * For weight-variant
         ...
         * fontWeightNormal
         */
        if ((/[a-z](Lighter|Light|Normal|Bold|Bolder)$/).test(propName))
            continue; // exclude
        // suffixes:
        /**
         * For state-variant
         ...
         * animInvalidInline
         */
        if ((/(None|Excited|Running|Enable|Disable|Active|Passive|Press|Release|Check|Clear|Hover|Arrive|Leave|Focus|Blur|Valid|Unvalid|Invalid|Uninvalid|Full|Compact)(Block|Inline)?$/).test(propName))
            continue; // exclude
         ...
         */
        if ((/^(backgGrad(Inline|Block)?|backg(Overlay|Striped)(Img|Size)?|orientation|align|horzAlign|vertAlign|spacing|img|size|valid|invalid|transDuration|(top|bottom|left|right)Transform|fontFamily\w+|fontSize[0-9]+)$/).test(propName))
            continue; // exclude
        // props starting with `@`:
        /**
         * Eg:
...
export const overwriteProps = (cssDecls, cssProps) => {
    ...
    for (const [propName, propValue] of Object.entries(cssProps)) {
        ...
        if (!targetPropName)
            continue; // target prop not found => skip
        result[targetPropName] = propValue;
    } // for
    return result;
};
/**
    ...
    for (const [propName, propValue] of Object.entries(cssProps)) {
        const targetPropName = (() => {
            for (const cssDecls of cssDeclss) {
                if (propName in cssDecls)
                    return cssDecls[propName]; // found => replace the cssDecl
            } // for
            return propName; // not found => use the original decl name
        })();
        if (!targetPropName)
            continue; // target prop not found => skip
        result[targetPropName] = propValue;
